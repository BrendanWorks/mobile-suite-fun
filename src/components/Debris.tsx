...
const Debris = forwardRef<GameHandle, DebrisProps>(({ onScoreUpdate, onComplete, timeRemaining }, ref) => {
  ...
  const addScore = useCallback((pts: number) => {
    ...
    onScoreUpdate?.(Math.round(scoreRef.current), MAX_SCORE);

    if (scoreRef.current >= MAX_SCORE && !wonRef.current && !doneRef.current) {
      wonRef.current = true;
      doneRef.current = true;
      cancelAnimationFrame(rafRef.current);
      setTimeout(() => onComplete?.(scoreRef.current, MAX_SCORE, timeRemaining), 400);
    }
  }, [onScoreUpdate, onComplete, timeRemaining]);

  const spawnParticles = useCallback((pos: Vec2, count: number, color: string, speed = 120) => {
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2;
  ...
  const destroyRock = useCallback((rock: Rock, rocks: Rock[]) => {
  ...
  const handlePlayerHit = useCallback(() => {
    ...
    if (livesRef.current <= 0 && !doneRef.current) {
      ...
      doneRef.current = true;
      cancelAnimationFrame(rafRef.current);
      setTimeout(() => onComplete?.(scoreRef.current, MAX_SCORE, 0), 500);
    }
  }, [spawnParticles, onComplete]);
  ...
  const fire = useCallback(() => {
    ...
    const px = playerPosRef.current.x + Math.cos(angle) * 16;
    const py = playerPosRef.current.y + Math.sin(angle) * 16;
  ...
  const gameLoop = useCallback((ts: number) => {
    ...
    if (targetWave > waveRef.current) {
      ...
      const extras = spawnWaveRocks(targetWave - 1, velocityBoost);
      rocksRef.current.push(...extras);
    }
  ...
  }, [fire, destroyRock, spawnParticles, handlePlayerHit]);
  ...
  const draw = useCallback(() => {
    ...
    const elapsed = Math.round((Date.now() - waveStartRef.current) / 1000);
  ...
  useEffect(() => {
    if (timeRemaining !== undefined && timeRemaining <= 0 && !doneRef.current) {
      doneRef.current = true;
      cancelAnimationFrame(rafRef.current);
      setTimeout(() => onComplete?.(scoreRef.current, MAX_SCORE, 0), 200);
    }
